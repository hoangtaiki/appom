...
RSpec.describe 'Phase 2 Integration Tests' do
  ...
  before do
    ...
    allow(mock_element).to receive(:text).and_return('Login')
  ...
  describe 'Element Caching Integration' do
    ...
    it 'improves performance through intelligent caching' do
      page = cached_page.new(mock_driver)
  ...
  describe 'Complete Workflow Integration' do
    it 'demonstrates full Phase 2 feature integration', :slow do
      ...
      # Cleanup
      [metrics_file, tracking_file].each { |file| File.delete(file) if File.exist?(file) }
    end
    ...
    it 'handles errors gracefully across all systems' do
      ...
      expect do
        result = Appom::Performance.time_operation('failing_workflow') do
          element_id = Appom::ElementState.track_element(page.login_button, name: 'failing_element')
  ...
  describe 'Configuration Integration' do
    ...
    after do
      File.delete('test_appom_config.yml') if File.exist?('test_appom_config.yml')
    end
