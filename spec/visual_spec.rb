...
RSpec.describe Appom::Visual do
  ...
  before do
    ...
    allow(mock_element).to receive_messages(location: { x: 100, y: 200 }, size: { width: 80, height: 30 })
  end
  ...
  describe Appom::Visual::TestHelpers do
    describe '#initialize' do
      ...
      it 'accepts custom configuration' do
        custom_helpers = described_class.new(
    ...
    describe '#visual_regression_test' do
      ...
      context 'when baseline exists' do
        ...
        it 'supports custom baseline path' do
          ...
          # Clean up
          FileUtils.rm_f(custom_baseline)
        end
      end
    end
    ...
    describe '#compare_element_visuals' do
      ...
      context 'when baseline exists' do
        ...
        it 'compares element with baseline' do
          # Create a mock baseline file
          File.join(test_helpers.baseline_dir, "#{baseline_name}_element.png")
    ...
    describe '#visual_diff' do
      ...
      before do
        ...
        allow(test_helpers).to receive(:compare_images).and_return({
                                                                     similarity: 0.85,
                                                                     differences: %w[color_difference size_difference],
                                                                   })
      end

      after do
        [image1_path, image2_path].each { |path| FileUtils.rm_f(path) }
      end
    ...
    describe '#highlight_element' do
      before do
        allow(test_helpers).to receive_messages(take_screenshot: 'temp_screenshot.png', annotate_screenshot: 'highlighted.png')
      end
    ...
    describe '#capture_element_sequence' do
      before do
        allow(test_helpers).to receive(:take_element_screenshot) do |_element, name|
          "#{name}.png"
        end
      end
    ...
    describe '#results_summary' do
      before do
        ...
        allow(test_helpers).to receive(:take_screenshot) { |path|
  ...
  describe 'Global Visual module' do
    before do
      described_class.clear_results!
      allow(described_class.test_helpers).to receive(:take_screenshot) do |path|
    ...
    describe '.regression_test' do
      it 'uses global test helpers' do
        allow(described_class.test_helpers).to receive(:visual_regression_test).and_return({
                                                                                             test_name: 'global_test',
                                                                                             passed: true,
                                                                                           })

        result = described_class.regression_test('global_test')
    ...
    describe '.results_summary' do
      it 'returns global results summary' do
        allow(described_class.test_helpers).to receive(:results_summary).and_return({
                                                                                      tests_run: 1,
                                                                                      passed: 1,
                                                                                      failed: 0,
                                                                                    })

        summary = described_class.results_summary
  ...
  describe Appom::Visual::DSL do
    ...
    before do
      allow(test_class.visual_test_helper).to receive_messages(visual_regression_test: { passed: true }, take_visual_screenshot: 'screenshot.png')
    end
