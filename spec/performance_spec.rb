  ...
  describe 'Global Performance module' do
    ...
    describe '.time_operation' do
      it 'uses the global monitor' do
        result = described_class.time_operation('global_test') do
          'result'
        end

        expect(result).to eq('result')
        expect(described_class.stats).to have_key('global_test')
      end
    end

    describe '.summary' do
      it 'returns global performance summary' do
        described_class.record_metric('test', 0.1, success: true)
    ...
    describe '.export_metrics' do
      it 'exports global metrics' do
        described_class.record_metric('export', 0.1, success: true)
        file_path = described_class.export_metrics(format: :json, file_path: 'global_test.json')
