module Appom
  module ElementState
    ...
    class Tracker
      ...
      def wait_for_state_change(element_id, expected_changes: {}, timeout: 10, interval: 0.5)
        ...
        loop do
          ...
          if expected_changes.all? { |key, value| current_state&.dig(key) == value }
      ...
      def find_elements_by_state(criteria)
        @tracked_elements.select do |element_id, tracked|
      ...
      def capture_element_state(element)
        ...
        begin
          ...
          if state[:exists]
            ...
            if element.respond_to?(:location)
              state[:location] = element.location
            end
            
            if element.respond_to?(:size)
      ...
      def state_changed?(old_state, new_state)
        ...
        comparison_keys = [:exists, :displayed, :enabled, :selected, :text, :attributes, :location, :size]
      ...
      def most_active_elements(limit)
        @tracked_elements
          .map { |id, tracked| [tracked[:name], tracked[:change_count]] }
    ...
    module Monitoring
      ...
      def wait_for_state_change(**options)
        return unless self.class.state_tracking_enabled?
        
        ElementState.tracker.wait_for_state_change(element_id, **options) if defined?(@element_id)
      end
    ...
    class << self
      ...
      # Convenience methods
      def track_element(element, **options)
        tracker.track_element(element, **options)
      end
      ...
      def wait_for_state_change(element_id, **options)
        tracker.wait_for_state_change(element_id, **options)
      end
      ...
      def export_data(**options)
        tracker.export_tracking_data(**options)
      end
    end
  end
end