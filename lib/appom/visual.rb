  ...
  module Visual
    ...
    class TestHelpers
      ...
      def visual_regression_test(test_name, element: nil, full_page: false, baseline: nil)
        ...
        screenshot_result = take_screenshot(current_path, element: element, full_page: full_page)
      ...
      def compare_images(image1_path, image2_path, diff_path = nil)
        ...
        @comparison_results.map do |result|
          ...
          <<~HTML
            <div class="test-result #{status_class}">
              ...
              <div class="images">
                #{File.exist?(result[:baseline_path] || '') ? "<div><h4>Baseline</h4><img src='file://#{result[:baseline_path]}' alt='Baseline'></div>" : ''}
                #{File.exist?(result[:current_path] || '') ? "<div><h4>Current</h4><img src='file://#{result[:current_path]}' alt='Current'></div>" : ''}
                #{result[:diff_path] && File.exist?(result[:diff_path]) ? "<div><h4>Difference</h4><img src='file://#{result[:diff_path]}' alt='Diff'></div>" : ''}
              </div>
            </div>
          HTML
        end.join("\n")
    ...
    module DSL
      ...
      def visual_regression_test(name, **options)
        self.class.visual_test_helper.visual_regression_test(name, **options)
      end

      def visual_screenshot(name, **options)
        self.class.visual_test_helper.take_visual_screenshot(name, **options)
      end

      def compare_visuals(baseline_name, **options)
        self.class.visual_test_helper.compare_element_visuals(self, baseline_name, **options)
      end

      def wait_for_visual_stability(**options)
        self.class.visual_test_helper.wait_for_visual_stability(**options)
      end

      def highlight(**options)
        self.class.visual_test_helper.highlight_element(self, **options)
      end
    end
    ...
    class << self
      ...
      # Convenience methods
      def regression_test(name, **options)
        test_helpers.visual_regression_test(name, **options)
      end

      def take_screenshot(name, **options)
        test_helpers.take_visual_screenshot(name, **options)
      end

      def visual_diff(image1, image2, **options)
        test_helpers.visual_diff(image1, image2, **options)
      end

      def generate_report(**options)
        test_helpers.generate_report(**options)
      end
