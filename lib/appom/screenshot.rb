...
module Appom
  module Screenshot
    ...
    class ScreenshotManager
      ...
      DEFAULT_DIRECTORY = 'screenshots'.freeze
      DEFAULT_FORMAT = :png
      ...
      def capture_before_after(name, &action_block)
      ...
      def capture_sequence(name, interval: 1.0, max_duration: 10.0, &action_block)
    ...
    class ScreenshotComparison
      ...
      def compare(image1_path, image2_path, output_path: nil)
        ...
        begin
          ...
          if output_path && @highlight_differences
            generate_diff_image(img1, img2, output_path)
          end
    ...
    class << self
      ...
      # Convenience methods
      def capture(name = 'screenshot', **options)
        manager.capture(name, **options)
      end
      ...
      def capture_before_after(name, &block)
        manager.capture_before_after(name, &block)
      end

      def capture_sequence(name, **options, &block)
        manager.capture_sequence(name, **options, &block)
      end
      ...
      def compare(image1, image2, **options)
        ScreenshotComparison.new(**options).compare(image1, image2)
      end

      def similar?(image1, image2, **options)
        ScreenshotComparison.new(**options).similar?(image1, image2)
      end
    end
  end
end