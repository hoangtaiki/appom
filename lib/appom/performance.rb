module Appom
  module Performance
    ...
    class Monitor
      ...
      def summary
        ...
        
        {
          ...
          average_operation_time: total_operations > 0 ? total_duration / total_operations : 0,
      ...
      def check_regressions(baseline_file, threshold_percent = 20)
        ...
        @metrics.each do |name, current_metric|
          ...
          if current_avg > baseline_avg * (1 + threshold_percent / 100.0)
      ...
      def calculate_stats(metric)
        return {} unless metric && metric[:total_calls] > 0
      ...
      module ClassMethods
        ...
        def instrument_method(method_name, operation_name: nil)
          ...
          define_method(method_name) do |*args, &block|
            Performance.monitor.time_operation(operation_name) do
    ...
    class << self
      ...
      # Convenience methods
      def time_operation(name, context = {}, &block)
        monitor.time_operation(name, context, &block)
      end

      def record_metric(name, duration, **options)
        monitor.record_metric(name, duration, **options)
      end
      ...
      def export_metrics(**options)
        monitor.export_metrics(**options)
      end
