  ...
  module ElementCache
    ...
    class Cache
      ...
      def cleanup_expired
        ...
        @cache.each do |key, (element, timestamp)|
    ...
    module CacheAwareFinder
      def self.included(klass)
        ...
        if klass.method_defined?(:find_element)
          klass.send(:alias_method, :original_find_element, :find_element)
        end
        if klass.method_defined?(:find_elements)
      ...
      def find_element(strategy, locator, use_cache: true)
        ...
        return _find_without_cache(strategy, locator) unless use_cache && (Appom.cache_config[:enabled] rescue true)
        ...
        if cached
          ...
          if element
            Appom::ElementCache.cache.store(strategy, locator, element)
          end
          element
        end
      end
      ...
      def find_elements(strategy, locator, use_cache: true)
        # If original method doesn't exist, delegate to the page/driver
        unless respond_to?(:original_find_elements)
          return page.find_elements(strategy, locator)
        end
        
        return original_find_elements(strategy, locator) unless use_cache && (Appom.cache_config[:enabled] rescue true)
        ...
        if cached
          ...
          if elements
            Appom::ElementCache.cache.store(strategy, locator, elements)
          end
          elements
        end
      end
    ...
    def configure_cache(**options)
      @global_cache = Cache.new(**options)
    end
  end
end
